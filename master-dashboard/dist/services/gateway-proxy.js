...
class GatewayProxy {
    static proxy = http_proxy_1.default.createProxyServer({
        changeOrigin: true,
        ws: true,
        xfwd: true,
    });
    static init() {
        this.proxy.on('proxyRes', (proxyRes, req) => {
            if (proxyRes.headers && proxyRes.headers.location) {
                ...
                proxyRes.headers.location = proxyRes.headers.location.replace(/https?:\/\/127\.0\.0\.1:8080/g, `${proto}://${host}`);
            }
        });
     ...
     */
    static handleTenantRouting(req, res, next) {
        const hostHeader = (req.headers.host || '').split(':')[0].toLowerCase();
        // Check if host matches a known school subdomain or custom domain
        const school = master_db_1.MasterDb.getSchoolByDomain(hostHeader);
        if (!school) {
      ...
      `);
        ...
        return GatewayProxy.proxy.web(req, res, { target }, (err) => {
            ...
            const ssoUrl = sso_service_1.SsoService.getSsoRedirectUrl(req.headers.host || school.subdomain, school.slug, 1);
            res.status(200).send(`
        <!DOCTYPE html>
        <html lang="en">
        <head>
