...
class Book extends Admin_Controller
{
    ...
    public function create()
    {
        ...
        // Validation
        $this->form_validation->set_rules('book_title', $this->lang->line('book_title'), 'trim|required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['listbook'] = $this->book_model->listbook();
            ...
            if (!empty($post['perunitcost'])) {
                $perunitcost = convertCurrencyFormatToBaseAmount($post['perunitcost']);
            } else {
                $perunitcost = '';
            }
            ...
            $insertData = array(
                ...
                'description' => $post['description'] ?? '', // ⚠️ see note below
            );

            if (!empty($post['postdate'])) {
                $insertData['postdate'] = date(
                    'Y-m-d',
                    $this->customlib->datetostrtotime($post['postdate'])
                );
            } else {
                $insertData['postdate'] = null;
            }


            $this->book_model->addbooks($insertData);

            $this->session->set_flashdata(
                'msg',
                '<div class="alert alert-success text-left">' . $this->lang->line('success_message') . '</div>'
            );
    ...
    public function edit($id)
    {
        ...
        $data['editbook']   = $this->book_model->get($id);

        // ✅ Validation (removed xss_clean from here)
        $this->form_validation->set_rules('book_title', $this->lang->line('book_title'), 'trim|required');
        $this->form_validation->set_rules('perunitcost', $this->lang->line('book_price'), 'numeric');
        $this->form_validation->set_rules('qty', $this->lang->line('qty'), 'numeric');
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($post['postdate'])) {
                $updateData['postdate'] = date(
                    'Y-m-d',
                    $this->customlib->datetostrtotime($post['postdate'])
                );
            } else {
                $updateData['postdate'] = null;
            }

           
            $this->book_model->addbooks($updateData);
    ...
    public function getbooklist()
    {
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('books', 'can_edit')) {
                    $editbtn = "<a href='" . base_url() . "admin/book/edit/" . $value->id . "'   class='btn btn-primary btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('edit') . "'><i class='fa fa-pencil'></i></a>";
                }

                if ($this->rbac->hasPrivilege('books', 'can_delete')) {
                    $deletebtn = "<a onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . "  )' href='" . base_url() . "admin/book/delete/" . $value->id . "' class='btn btn-primary btn-xs' title='" . $this->lang->line('delete') . "' data-toggle='tooltip'><i class='fa fa-trash'></i></a>";
                }
