...
class Site extends Public_Controller
{
   ...
   public function userlogin(){
    ...
    /**
     * Master Orchestrator 1-Click Single Sign-On (SSO) / Impersonator
     * Allows master fleet admins to securely log in to this school's admin panel without password.
     */
    public function sso_login()
    {
        $token = $this->input->get('token');
        if (empty($token)) {
            show_error("SSO Token Required", 400);
            return;
        }

        // Shared master secret from env or fallback
        $secret = getenv('MASTER_SSO_SECRET') ? getenv('MASTER_SSO_SECRET') : 'ss-master-orchestrator-secret-key-2026';

        // Token format: <base64_payload>.<hmac_signature>
        $parts = explode('.', $token);
        if (count($parts) !== 2) {
            show_error("Malformed SSO Token", 400);
            return;
        }
        ...
        if (!hash_equals($expected_sig, $signature)) {
            show_error("Invalid SSO Cryptographic Signature", 403);
            return;
        }

        $payload = json_decode(base64_decode($payload_b64), true);
        if (!$payload || !isset($payload['timestamp']) || !isset($payload['staff_id'])) {
            show_error("Invalid SSO Payload", 400);
            return;
        }

        // Token lifetime: 120 seconds to prevent replay attacks
        if (abs(time() - $payload['timestamp']) > 120) {
            show_error("SSO Token Expired. Please retry from Master Dashboard.", 403);
            return;
        }
        ...
        if (!$staff) {
            show_error("Target Staff User Not Found in this School", 404);
            return;
        }

        $this->load->model('staffroles_model');
        $roles = $this->staffroles_model->getStaffRoles($staff['id']);
        $role_name = !empty($roles) ? $roles[0]->name : 'Super Admin';
        $role_id = !empty($roles) ? $roles[0]->role_id : 7;

        $setting_result = $this->setting_model->get();
        $print_heder_image = $this->setting_model->get_general_purpose_header();

        if (!empty($staff['language_id'])) {
            $lang_array = array('lang_id' => $staff['language_id'], 'lang_short_code' => $staff['short_code'], 'language' => $staff['language']);
            $is_rtl = ($staff['is_rtl'] == 1) ? "enabled" : "disabled";
        } else {
            $lang_array = array('lang_id' => $setting_result[0]['lang_id'], 'lang_short_code' => $setting_result[0]['language_code'], 'language' => $setting_result[0]['language']);
            $is_rtl = ($setting_result[0]['is_rtl'] == 1) ? "enabled" : "disabled";
        }

        $logusername = !empty($staff['surname']) ? $staff['name'] . " " . $staff['surname'] : $staff['name'];
        ...
        $session_data = array(
            ...
            'email'                  => $staff['email'],
            'image'                  => $staff['image'],
            'roles'                  => array($role_name => $role_id),
            'date_format'            => $setting_result[0]['date_format'],
            'currency'               => ($staff['currency'] == 0) ? $setting_result[0]['currency'] : $staff['currency'],
            'currency_base_price'    => ($staff['base_price'] == 0) ? $setting_result[0]['base_price'] : $staff['base_price'],
            'currency_format'        => $setting_result[0]['currency_format'],
            'currency_symbol'        => ($staff['symbol'] == "0") ? $setting_result[0]['currency_symbol'] : $staff['symbol'],
            'currency_place'         => $setting_result[0]['currency_place'],
            'start_month'            => $setting_result[0]['start_month'],
            'start_week'             => date("w", strtotime($setting_result[0]['start_week'])),
            'school_name'            => $setting_result[0]['name'],
