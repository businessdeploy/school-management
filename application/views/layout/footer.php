...
$print_header_image           = $this->customlib->getPrintHeaderImage();
...
<!-- drag and drop touch page js link added -->
<script src="<?php echo base_url(); ?>backend/js/jquery.draggableTouch.js"></script>
<!-- drag and drop touch page js link added -->
...
<script type="text/javascript">
    var imageUrl = "<?php echo !IsNullOrEmptyString($print_header_image) ? base_url("uploads/print_headerfooter/general_purpose/".$print_header_image): "";?>";
    $('body').tooltip({
...
<script>
                ...
                let custom_color = (themeSettings.theme_type === "custom")  ? themeSettings.theme_color  : '#f0852eff';
                const themes = [
                    '#7367f0',
                    '#2092EC',
                    '#FFAB1D',
    ...
    function toggleTheme() {
        ...
        // Remove both light and dark before toggling
        body.classList.remove('light-mode', 'dark');
        toggleBtn?.classList.remove('light-mode-button', 'dark-mode-button');
        themeIcon?.classList.remove('icon-sun', 'icon-moon-star');

        if (isDark) {
            // Switch to Light Mode
            body.classList.add('light-mode');
            toggleBtn?.classList.add('light-mode-button');
            themeIcon?.classList.add('icon-sun');
        } else {
            // Switch to Dark Mode
            body.classList.add('dark');
            toggleBtn?.classList.add('dark-mode-button');
            themeIcon?.classList.add('icon-moon-star');
        }
        document.querySelector('.theme_background').value = body.classList.contains('dark') ? 'dark' : 'light-mode';
        // Save preference
        localStorage.setItem('theme', body.classList.contains('dark') ? 'dark' : 'light-mode');
    }
    ...
    function setColorTheme(theme) {
        ...
        // Add selected theme if valid
        if (themes.includes(theme)) {
            const root = document.documentElement;        

            root.style.setProperty('--bs-primary', theme);
            root.style.setProperty('--bs-btn-border-color', theme);
            root.style.setProperty('--bs-primary-hover', theme);
            root.style.setProperty('--bs-hover-color', theme);
            root.style.setProperty('--bs-alert-bg', theme);
            root.style.setProperty('--custom-hover-theme', theme);
            root.style.setProperty('--bs-primary-contrast', '#fff');
     
         
        } else {
            console.warn(`Theme "${theme}" is not recognized.`);
        }

        // Update active state on theme buttons
        document.querySelectorAll('.theme-link').forEach(link => {
            console.log(link.dataset.theme +"==="+ theme);
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const icon = document.getElementById('content-icon');
    const layoutBtn = document.querySelector(".togglewide");
    const themeContent = document.querySelector('.theme_content');
    ...
    document.querySelectorAll('.theme-link').forEach(link => {
        link.addEventListener('click', () => {
            const selectedTheme = link.dataset.theme;
              ...
              if (link.classList.contains('colorPickerBtn')) {
            ...
            if (themeColorInput) {
                themeColorInput.value = selectedTheme;
            }

            setColorTheme(selectedTheme); // Assumes this function is defined elsewhere
        });
    });

    // 2. Layout toggle button
    if (layoutBtn && content) {
        layoutBtn.addEventListener("click", (e) => {
            e.preventDefault();

            const isFluid = content.classList.contains("container-fluid");

            // Reset both layout classes
            content.classList.remove("container-fluid", "container-xxl");

            // Toggle layout class
            const newLayout = isFluid ? "container-xxl" : "container-fluid";
            content.classList.add(newLayout);

            // Update hidden input if used for saving preferences
            if (themeContent) {
                themeContent.value = newLayout;
            }

            // Update icon
            if (icon) {
                icon.classList.toggle('icon-maximize', newLayout === 'container-fluid');
                icon.classList.toggle('icon-minimize', newLayout === 'container-xxl');
            }
        });
    }
});
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const themeShadowBtn = document.querySelector('.theme_shadow_btn');

        const icon = document.getElementById('iconskins');
        const themeInput = document.querySelector('.theme_shadow');

        if (!themeShadowBtn || !icon) return; // Safety check: don't continue if elements are missing

        themeShadowBtn.addEventListener('click', function() {
            const body = document.body;

            // Toggle the shadow class
            const isApplied = body.classList.toggle('shadow-applied');

            // Update the input or text element with the current value
            if (themeInput) {
                const value = isApplied ? 'shadow-applied' : '';
                if ('value' in themeInput) {
                    themeInput.value = value;
                } else {
                    themeInput.textContent = value;
                }
            }

            // Toggle Font Awesome icon classes
            if (icon.classList.contains('icon-grid-2x2')) {
                icon.classList.replace('icon-grid-2x2', 'bi-shadows');
            } else {
                icon.classList.replace('bi-shadows', 'icon-grid-2x2');
            }
        });
    });
</script>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const themeNavigationBtn = document.querySelector('.theme_navigation_btn');

        const icon = document.getElementById('icon_theme_navigation');
        const themeNavigation = document.querySelector('.theme_navigation');

        if (!themeNavigationBtn || !icon) return; // Safety check: don't continue if elements are missing

        themeNavigationBtn.addEventListener('click', function() {
            const body = document.body;

            // Toggle the shadow class
            const isApplied = body.classList.toggle('sidebar-collapse');

            // Update the input or text element with the current value
            if (themeNavigation) {
                const value = isApplied ? 'collapsed' : 'expanded';
                if ('value' in themeNavigation) {
                    themeNavigation.value = value;
                } else {
                    themeNavigation.textContent = value;
                }
            }

        if (icon.classList.contains('icon-unfold-horizontal')) {
    // Remove old icon style and icon name
    icon.classList.remove('icon-unfold-horizontal');

    // Add new style and icon name
    icon.classList.add('fa-brands', 'icon-fold-horizontal');
} else {
    // Revert back to original
    icon.classList.remove('fa-brands', 'icon-fold-horizontal');
    icon.classList.add('icon-unfold-horizontal');
}
        });
    });
</script>
...
<script type="text/javascript" src="<?php echo base_url(); ?>backend/dist/datatables/2.2.2/pdfmake.min.js"></script>
<script type="text/javascript"> //for pdf
 
    var json_pdf_data = <?php echo json_encode($this->customlib->getLanguagePDFJson()); ?>;

var jsonData = json_pdf_data.lang_arr_json;
var pdf_language_font = json_pdf_data.pdf_language_font;
var pdf_language_align = (<?php echo $this->customlib->checkSessionLanguageRTL(); ?> == 1) ? 'right' : 'left';
pdfMake.fonts = jsonData;
 
  </script>


<script type="text/javascript" src="<?php echo base_url(); ?>backend/dist/datatables/2.2.2/dataTables.js"></script>
<script type="text/javascript" src="<?php echo base_url(); ?>backend/dist/datatables/2.2.2/dataTables.buttons.js"></script>
<script type="text/javascript" src="<?php echo base_url(); ?>backend/dist/datatables/2.2.2/buttons.dataTables.js"></script>
<script type="text/javascript" src="<?php echo base_url(); ?>backend/dist/datatables/2.2.2/jszip.min.js"></script>
<script type="text/javascript" src="<?php echo base_url(); ?>backend/dist/datatables/2.2.2/vfs_fonts.js"></script>
<script type="text/javascript" src="<?php echo base_url(); ?>backend/dist/datatables/2.2.2/buttons.html5.min.js"></script>
<link rel="stylesheet" type="text/css" href="<?php echo base_url(); ?>backend/dist/datatables/2.2.2/dataTables.dataTables.css">
<link rel="stylesheet" type="text/css" href="<?php echo base_url(); ?>backend/dist/datatables/2.2.2/buttons.dataTables.css">
<link rel="stylesheet" type="text/css" href="<?php echo base_url(); ?>backend/dist/datatables/2.2.2/ss.custom.css">
<script type="text/javascript" src="<?php echo base_url(); ?>backend/dist/datatables/2.2.2/buttons.print.min.js"></script>
<script type="text/javascript" src="<?php echo base_url(); ?>backend/dist/datatables/2.2.2/buttons.colVis.min.js" ></script>
<script type="text/javascript" src="<?php echo base_url(); ?>backend/dist/datatables/2.2.2/ss.custom.js" ></script>
 <script type="text/javascript">
        $(document).ready(function () {
            // Function to adjust position
            // Function to adjust position
            function adjustSubmenuPosition($item) {
                var $submenu = $item.find('.treeview-menu');
                var $label = $item.find('> a > span');

                // Only proceed if sidebar is collapsed
                if (!$('body').hasClass('sidebar-collapse')) return;

                if ($submenu.length || $label.length) {
                    var itemRect = $item[0].getBoundingClientRect();
                    var top = itemRect.top;
                    var left = 65; // Fixed width of collapsed sidebar

                    if ($submenu.length) {
                        // Measure height first
                        $submenu.css({ 'display': 'block', 'visibility': 'hidden' });
                        var height = $submenu.outerHeight();
                        $submenu.css({ 'visibility': 'visible' }); // Restore visibility

                        var windowHeight = $(window).height();
                        var bottomOverflow = (top + 46 + height) - windowHeight;
                        ...
                        if (bottomOverflow > 0) {
                            ...
                            // Top position would be top - height
                            if (top - height > 0) {
                                finalTop = top - height;
                            } else {
                                // Fits neither? Use scroll below
                                useScroll = true;
                            }
                        }

                        if (useScroll) {
                            var availableHeight = windowHeight - finalTop - 10;
                            $submenu.css({
                                'position': 'fixed',
                                'top': finalTop + 'px',
                                'left': left + 'px',
                    ...
                    if ($label.length) {
                        $label.css({
                            ...
                            'left': left + 'px',
                            'display': 'block',
                            'z-index': '10001', // Ensure above menu
                            'width': '200px', // Match menu width if it's acting as a header
            ...
            $(document).on('mouseenter', '.sidebar-mini.sidebar-collapse .sidebar-menu > li', function () {
                adjustSubmenuPosition($(this));
            });

            $(document).on('mouseleave', '.sidebar-mini.sidebar-collapse .sidebar-menu > li', function () {
                var $submenu = $(this).find('.treeview-menu');
                var $label = $(this).find('> a > span');
...
//===============update chat notification================//
    get_chat_msg_count();
    setInterval(get_chat_msg_count,15000);
    function get_chat_msg_count(){
        $.ajax({
            type: "POST",
            url: base_url + 'admin/chat/get_chat_msg_count',
            dataType: "JSON",
            data: { },
            success: function (data) {
				
				if(data.status == 1 && data.count > 0){
					$(".total_chat_msg").text(data.count);
				}
