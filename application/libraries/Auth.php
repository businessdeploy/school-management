...
class Auth
{
    ...
    public function app_update()
    {
        ...
        if (file_exists($fname)) {
            ...
            $file_contents = str_replace('$config[\'SSLK\'] = \'\'', '$config[\'SSLK\'] = \'SS-ACTIVE-PRO-LICENSE\'', $content);
    ...
    public function addon_update()
    {
        ...
        if (!empty($addon)) {
            $this->CI->db->where('short_name', $addon)->update('addons', array(
                'addon_ver' => '1.0.0',
                'addon_prod' => 'VERIFIED'
            ));
            $this->CI->db->where('product_name', $addon)->update('sidebar_menus', array(
                'is_active' => 1
            ));
        }
